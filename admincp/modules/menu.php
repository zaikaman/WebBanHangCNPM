<!-- Admin Sidebar Menu -->
<div class="admin-sidebar">
    <ul class="nav flex-column">
        <li class="nav-item">
            <a class="nav-link" href="index.php">
                <i class="fas fa-chart-line"></i> Dashboard
            </a>
        </li>
        <li class="nav-item">
            <a class="nav-link" href="index.php?action=quanLyDanhMucSanPham&query=them">
                <i class="fas fa-layer-group"></i> Quản lý danh mục sản phẩm
            </a>
        </li>
        <li class="nav-item">
            <a class="nav-link" href="index.php?action=quanLySanPham&query=lietke">
                <i class="fas fa-tshirt"></i> Quản lý sản phẩm
            </a>
        </li>
        <li class="nav-item">
            <a class="nav-link" href="index.php?action=quanLyDanhMucBaiViet&query=them">
                <i class="fas fa-folder-open"></i> Quản lý danh mục bài viết
            </a>
        </li>
        <li class="nav-item">
            <a class="nav-link" href="index.php?action=quanLyBaiViet&query=them">
                <i class="fas fa-edit"></i> Quản lý bài viết
            </a>
        </li>
        <li class="nav-item">
            <a class="nav-link" href="index.php?action=quanLyDonHang&query=lietke">
                <i class="fas fa-shopping-cart"></i> Quản lý đơn hàng
            </a>
        </li>
        <li class="nav-item">
            <a class="nav-link" href="index.php?action=quanLyWeb">
                <i class="fas fa-cogs"></i> Quản lý website
            </a>
        </li>
        <li class="nav-item">
            <a class="nav-link" href="index.php?action=quanLyTaiKhoanKhachHang&query=lietke">
                <i class="fas fa-users"></i> Quản lý khách hàng
            </a>
        </li>
    </ul>
</div>
