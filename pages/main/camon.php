...
if (isset($_GET['partnerCode'])) {
    ...
    $vnp_ResponseCode = isset($_GET['vnp_ResponseCode']) ? $_GET['vnp_ResponseCode'] : '-1';
    
    // vnp_ResponseCode = 00 nghĩa là giao dịch thành công
    if ($vnp_ResponseCode != '00') {
        $payment_success = false;
        $payment_message = 'Giao dịch VNPay thất bại hoặc bị hủy. Mã lỗi: ' . $vnp_ResponseCode;
        error_log('VNPay Payment Failed - vnp_ResponseCode: ' . $vnp_ResponseCode . ' - TxnRef: ' . ($_GET['vnp_TxnRef'] ?? 'N/A'), 3, __DIR__ . '/payment-error.log');
    } else {
        $payment_success = true;
        ...
        if ($cart_query) {
            ...
            error_log('VNPay DB Insert Failed - code_cart: ' . $code_cart, 3, __DIR__ . '/payment-error.log');
        }
